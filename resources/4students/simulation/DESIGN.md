...
### 1. `sim_recorder.py` — headless episode recording

Records simulation state to a compressed `.npz` array (poses, LiDAR, velocities, steps).
Lightweight, fast, no rendering overhead. Output: `./recordings/episode_<timestamp>.npz`.

```bash
# Record an episode (headless, max speed):
docker compose run --rm recorder
docker compose run --rm recorder python sim_recorder.py --steps 1000 --num-cars 2 --out /app/episode.npz
```

### 2. `viz_replay.py` — top-down replay from recorded episode

Plots saved poses over the occupancy grid, animating the race with configurable follow-camera.
Supports both Agg (PNG snapshot) and TkAgg (interactive window).

```bash
# Headless PNG snapshot of a full race:
docker compose run --rm replay python viz_replay.py --npz /app/episode.npz --no-display

# Interactive replay with follow-camera on car 0:
xhost +local:docker
docker compose run --rm -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix \
  replay python viz_replay.py --npz /app/episode.npz --follow 0
```
...
| C4 | `viz_replay.py` matplotlib top-down replay (Agg/TkAgg) | ✅ |
| C5 | `sim_recorder.py` episode recording (.npz) | ✅ |
