...
_DNF_PROGRESS_THRESH = 0.01      # minimum cumulative progress change over window (fraction of track, 1% ≈ 0.15m on ~15m track)
_LIDAR_RAYS        = 100         # beam count (class-var override after every build)
_LIDAR_FOV         = 4.7         # field of view (rad)
_LIDAR_NOISE       = 0.03        # multiplicative uniform noise half-range (±3%)
...
class _Sim:
    ...
    def __init__(self) -> None:
        ...
        self._wall_pinned: np.ndarray = np.zeros(_MAX_SLOTS, dtype=bool)
    ...
    def reset(self, num_cars: int) -> None:
        ...
        for i in range(_MAX_SLOTS):
            ...
            self._wall_pinned[i]     = False          # wall pinning reset
    ...
    def apply_action(self, agent_id: int, target_speed: float, steering: float) -> None:
        ...
        if self._wall_pinned[agent_id]:
            if speed >= 0:
                return  # forward ignored while pinned
            else:
                self._wall_pinned[agent_id] = False  # reverse unpins
    ...
    def simulation_step(self) -> None:
        ...
        for sub in range(_PHYSICS_STEPS):
            ...
            for i in range(n):
                ...
                if wall:
                    # Micro-teleport: restore pose AND push 5 cm backward along heading.
                    # This moves the car clearly outside the iTTC detection zone so it
                    # stays still next sub-step instead of immediately re-triggering.
                    self._sim.agents[i].state[0] = snap_x[i] - 0.35 * math.cos(snap_yaw[i])
                    self._sim.agents[i].state[1] = snap_y[i] - 0.35 * math.sin(snap_yaw[i])
                    self._sim.agents[i].state[4] = snap_yaw[i]
                    # vel/yaw_rate/slip already zeroed by engine check_ttc — leave them at 0
                    ctrl[i, 1] = 0.0
                    self._wall_pinned[i] = True  # block forward commands until reverse
    ...
    def _update_friction(self) -> None:
        ...
        if self._friction_countdown <= 0:
            lo, hi = _FRICTION_LO, _FRICTION_HI
            self._friction = float(self._rng.uniform(lo, hi))
            p = dict(_PARAMS)
            p["mu"] = self._friction
    ...
    def _update_stagnation_and_dnf(self) -> None:
        ...
        for i in range(n):
            ...
            # Exception: skip stagnation freeze while the car is pinned to a wall
            # (wall pinning keeps it motionless by design — DESIGN.md § "Wall response").
            current_wall_hit = (self._wall_hit[i] and self._status[i] == 1)
...
def apply_action(agent_id: int, target_speed: float, steering: float) -> None:
    ...
    Silently clamped: speed ∈ [-0.6, 5.0] m/s, steering ∈ [-0.4189, 0.4189] rad.
...
def set_map(name: str, num_cars: int | None = None) -> None:
    ...
    _current_map = name
    # Sync into the active _Sim singleton so it knows what's loaded
    if _sim_instance is not None:
...
def _rebuild_waypoints_from_csv(csv_path: str) -> None:
    """Rebuild _waypoints, _arc, _total_arc from a centerline CSV (njit-compatible)."""
    data = np.loadtxt(csv_path, delimiter=",", skiprows=0)
    # Handle comment lines by filtering them out first.
    rows = []
    ...
    wpts = np.array(rows, dtype=np.float64)
    diffs = np.diff(wpts, axis=0)
    seg_lens = np.sqrt((diffs ** 2).sum(axis=1))
    arc = np.concatenate([[0.0], np.cumsum(seg_lens)])
    _Sim._waypoints = np.ascontiguousarray(wpts, dtype=np.float64)
    _Sim._arc = arc
    _Sim._total_arc = arc[-1]
